...
def feed(request):
    ...
    filter_date = request.GET.get('date', 'latest')  # Date filter
    filter_media = request.GET.get('media', 'all')  # Media type filter
    filter_user = request.GET.get('user', '')  # Author filter
    ...
    if filter_user:
        posts = posts.filter(user__username=filter_user)
    ...
    if request.user.is_authenticated:
        ...
        return render(request, 'guest_feed.html', context)
@login_required
def post_create(request):
    if request.method == 'POST':
